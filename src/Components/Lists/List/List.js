...
import * as actions from 'store/actions/favLists.js'
...
const List = (props) => {
  ...
  if (Object.keys(props.favLists).length > 0 && props.favLists[props.id]) {
    button = <button className="btn border-secondary" onClick={() => props.onfavListsRemove(props.id)}>Remove</button>;
  }else{
    button = <button className="btn btn-secondary" onClick={() => props.onfavListsAdd(props.id, {
      "Title": props.title,
      "Year": props.year,
      "imdbID": props.id,
      "Type": props.type,
